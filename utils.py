...
def get_scheduler(config, optimizer, num_step, epochs):
    assert config.sch in ['StepLR', 'MultiStepLR', 'ExponentialLR', 'CosineAnnealingLR', 'ReduceLROnPlateau',
                          ...
                          'CustomLR'], 'Unsupported scheduler!'
...
class GT_BceDiceLoss(nn.Module):
    ...
    def forward(self, gt_pre, out, target):
        ...
        gt_pre5, gt_pre2, gt_pre1 = gt_pre

        # print('5=====',gt_pre5.shape)
        # print('1=====',gt_pre1.shape)
        ...
        gt_loss = self.bcedice(gt_pre5, target) * 0.1 + self.bcedice(
            gt_pre2, target) * 0.4 + self.bcedice(gt_pre1, target) * 0.5
